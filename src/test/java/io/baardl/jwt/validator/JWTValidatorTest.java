...
public class JWTValidatorTest {
    ...
    public void setUp() throws Exception {
     userToken = Configuration.getString("base64UrlEncodedUserToken"); //Insert a valid base64UrlEncoded token in your jwtValidation_override.properties file.
    }

    @Test
    public void validateUserToken() {
        assertTrue("You might need to insert a valid userToken", JWTValidator.validateUserToken(userToken,jwksJson));
    }

    @Test
    public void parseJWT() {
        JsonWebSignature jws = JWTValidator.buildJwsFromUserToken(userToken);
        JsonWebKey publicKey = JWTValidator.buildKey(jwksJson, jws);
        assertNotNull("Public key must be found", publicKey);
        JwtClaims expectedClaims = null;
        JwtClaims foundClaims = JWTValidator.parseJWT(userToken, publicKey);
        assertEquals(expectedClaims, foundClaims);
    }
}